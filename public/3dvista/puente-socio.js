(function () {
  ...
  var RE_DIA=new RegExp('[\\u0300-\\u036f]','g');
  var RE_ESP=new RegExp('\\s+','g');
  function norm(s){ return String(s||'').toLowerCase().normalize('NFD').replace(RE_DIA,'').replace(RE_ESP,'-').trim(); }
  ...
  // ESTADISTICAS -- arranca en diferido y aislado; NUNCA usa regex literales
  setTimeout(function(){
    try {
      var TRACK_URL='https://mendoza-bureau.vercel.app/api/track';
      var RE_IRA=new RegExp('/tour/ir-a/([A-Za-z0-9_-]+)');
      var RE_ID=new RegExp('[?&]id=([A-Za-z0-9_-]+)');
      var sidCache=null;
      var detId=function(){
        if(sidCache)return sidCache;
        var busc=function(u){ if(!u)return null; var m=String(u).match(RE_IRA)||String(u).match(RE_ID); return m?m[1]:null; };
