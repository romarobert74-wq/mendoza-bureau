// @ts-nocheck
'use client'
/* SplashCursor — efecto de fluido WebGL (React Bits · DavidHDev/react-bits, MIT).
   Adaptado para Mendoza Bureau: se acota al contenedor padre (hero) en vez de
   ocupar toda la pantalla, va detrás del texto y no bloquea clics. */
...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function calcDeltaTime() {
      let now = Date.now(); let dt = (now - lastUpdateTime) / 1000
    ...
    // Coordenadas relativas al canvas (para que funcione acotado al hero)
    function rectPos(e) {
    ...
    function handleMouseMove(e) {
      ...
      const { x, y } = rectPos(e)
      let posX = scaleByPixelRatio(x); let posY = scaleByPixelRatio(y)
      let color = pointer.color && (pointer.color.r || pointer.color.g || pointer.color.b) ? pointer.color : generateColor()
