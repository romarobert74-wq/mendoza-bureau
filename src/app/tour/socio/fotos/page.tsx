...
export default function SocioFotosPage() {
  ...
  if (fotos.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-3" style={{ background: 'rgba(10,15,30,0.93)', fontFamily: 'system-ui, sans-serif' }}>
  ...
  return (
    ...
    >
      ...
      {/* Foto principal */}
      <div
        className="relative flex-1 mx-4 rounded-2xl overflow-hidden cursor-pointer"
        style={{ minHeight: '55vh', background: '#111827' }}
        onClick={() => setFullscreen(true)}
      >
        <img
          src={fotos[current].url}
          alt={fotos[current].nombre}
        ...
        <div className="flex gap-2 px-4 py-4 overflow-x-auto">
          {fotos.map((f, i) => (
            <button
              ...
              style={{
                border: i === current ? '2px solid #60a5fa' : '2px solid transparent',
