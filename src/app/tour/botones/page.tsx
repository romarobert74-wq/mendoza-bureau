      ...
      <div className="stack">
        <div className="col">
          <button className="halo info" onClick={() => abrir(destInfo)} aria-label="Información">
            <span className="ic"><Info size={20} /></span>
          </button>
...
const CSS = `
  ...
  /* OPCIÓN 2 · Halo Neón (tamaño reducido) */
  .halo{ position:relative; width:52px; height:52px; border-radius:999px; cursor:pointer;
  ...
  .halo.info{ --c:var(--info); box-shadow:0 0 0 2px var(--info), 0 0 14px -3px var(--info); }
  .halo.ir{ --c:var(--ir); box-shadow:0 0 0 2px var(--ir), 0 0 14px -3px var(--ir); }
  .halo .ic{ display:grid; place-items:center; transition:.2s; }
  .halo:hover{ box-shadow:0 0 0 2px var(--c), 0 0 28px 1px var(--c); }
  .halo:hover .ic{ animation:boing .5s ease; color:#fff; }
  .halo:active{ transform:scale(.93); }

  .cap{ font-size:10.5px; font-weight:700; letter-spacing:.03em; color:#e8eef7; text-transform:uppercase;
