  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-[var(--text)]">Tour Madre</h2>
          <p className="text-[var(--text-muted)] text-sm mt-1">Configuración del menú del tour madre</p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {/* Info Bureau */}
          <section className="bg-[var(--bg-elev)] rounded-xl border border-[var(--border)] p-6">
            <h3 className="font-semibold text-[var(--text)] mb-4">Info Bureau (skin madre)</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-[var(--text-2)] mb-1">Teléfono / WhatsApp Bureau</label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--text-2)] mb-1">Email Bureau</label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--text-2)] mb-1">Ubicación</label>
                <input
          ...
          <section className="bg-[var(--bg-elev)] rounded-xl border border-[var(--border)] p-6">
            <h3 className="font-semibold text-[var(--text)] mb-1">Socios activos en el tour</h3>
            ...
            {porCategoria.map(({ cat, label, items }) => (
              <div key={cat} className="mb-4">
                ...
                <div className="space-y-1">
                  {items.map(s => (
                    <div key={s.id} className="flex items-center justify-between text-sm bg-[var(--bg-input)] rounded-lg px-3 py-2">
                      <span className="font-medium text-[var(--text)]">{s.etiqueta}</span>
                      <span className="text-[var(--text-muted)] text-xs truncate max-w-xs">{s.urlInternaTour || 'Sin URL interna'}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {porCategoria.length === 0 && (
              <p className="text-[var(--text-muted)] text-sm">No hay socios activos</p>
            )}
          </section>
        </div>
      )}
